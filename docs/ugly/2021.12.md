...
参考地址：<https://xiaolincoding.com/network/3_tcp/tcp_http_keepalive.html#tcp-%E7%9A%84-keepalive>
...
- `async` 脚本和 `defer` 脚本：`defer` 脚本始终在 `DCL` 之前执行。如果 `async` 脚本在前，其脚本的执行时间跟脚本的执行时长有关系，在 `DCL` 前后都有；而大部分 `async` 脚本在后，其脚本执行时间都在 `DCL` 后
